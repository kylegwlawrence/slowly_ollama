...
# ---------------------------------------------------------------------------
# generate_json (Phase 24 — degree factory's low-context primitive)
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_generate_json_returns_parsed_dict() -> None:
    """A non-streaming /api/chat whose content is a JSON object round-trips
    to a dict; the request carries stream=False and the system+user pair."""
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            200,
            json={"message": {"content": json.dumps({"slug": "x", "n": 1})}},
        )

    async with _client_with(handler) as client:
        result = await generate_json(
            client, "m", system="rules", user="context"
        )

    assert result == {"slug": "x", "n": 1}


@pytest.mark.asyncio
async def test_generate_json_passes_format_when_schema_given() -> None:
    """A provided format_schema is forwarded as the top-level `format` key
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            200, json={"message": {"content": json.dumps({"slug": "x"})}}
        )

    async with _client_with(handler) as client:
        await generate_json(
            client, "m", system="s", user="u", format_schema=schema
        )
...
@pytest.mark.asyncio
async def test_generate_json_omits_format_when_none() -> None:
    """No format_schema → no `format` key in the payload."""
    captured: dict = {}

    def handler(request: httpx.Request) -> httpx.Response:
        captured.update(json.loads(request.content))
        return httpx.Response(
            200, json={"message": {"content": json.dumps({"ok": True})}}
        )

    async with _client_with(handler) as client:
        await generate_json(client, "m", system="s", user="u")

    assert "format" not in captured


@pytest.mark.asyncio
async def test_generate_json_passes_num_ctx_when_given() -> None:
    """num_ctx lands in options; absent otherwise."""
    captured: dict = {}
    ...
    async with _client_with(handler) as client:
        await generate_json(client, "m", system="s", user="u", num_ctx=2048)

    assert captured["options"]["num_ctx"] == 2048


@pytest.mark.asyncio
async def test_generate_json_bad_json_content_raises_protocol_error() -> None:
    """Content that isn't valid JSON surfaces as OllamaProtocolError."""
...
@pytest.mark.asyncio
async def test_generate_json_non_object_json_raises_protocol_error() -> None:
    """Valid JSON that isn't an object (e.g. a list) is rejected — the
    contract is a dict."""

    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(
            200, json={"message": {"content": "[1, 2, 3]"}}
        )
...
@pytest.mark.asyncio
async def test_generate_json_malformed_envelope_raises_protocol_error() -> None:
    """A /api/chat body missing message.content is a protocol error."""

    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(200, json={"wrong_key": 1})
...
@pytest.mark.asyncio
async def test_generate_json_transport_failure_raises_unavailable() -> None:
    """A connect error is wrapped as OllamaUnavailable."""
...
@pytest.mark.asyncio
async def test_generate_json_5xx_raises_unavailable() -> None:
    """A non-2xx status is OllamaUnavailable."""
