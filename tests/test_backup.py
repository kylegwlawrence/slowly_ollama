...
Hermetic and mock-only — no real ssh/rsync, no network, no pop-os. The
...
def _enable(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("REMOTE_PATH", "pop-os:/remote/agent_workspaces")
    monkeypatch.setenv("REMOTE_DB_PATH", "pop-os:/remote/olliellama_chats")
...
async def test_push_database_targets_chats_db_and_never_wal(
    ...
    ok = await backup._push_database("pop-os", "/remote/olliellama_chats")

    assert ok is True
    rsync = next(c for c in commands if c[0] == "rsync")
    assert rsync[-1] == "pop-os:/remote/olliellama_chats/chats.db"
...
async def test_push_database_noops_without_db_file(
    ...
    ok = await backup._push_database("pop-os", "/remote/db")
...
async def test_maybe_snapshot_runs_cp_and_records_when_due(
    ...
    await backup._maybe_snapshot(
        "pop-os", "/remote/db", "pop-os", "/remote/ws",
        {"pop-os": True},
    )
...
async def test_maybe_snapshot_skips_when_not_due(
    ...
    await backup._maybe_snapshot(
        "pop-os", "/remote/db", "pop-os", "/remote/ws", {"pop-os": True}
    )
...
async def test_push_database_bails_when_mkdir_fails(
    ...
    assert await backup._push_database("pop-os", "/remote/db") is False
...
async def test_push_workspaces_syncs_with_trailing_slash(
    ...
    ok = await backup._push_workspaces("pop-os", "/remote/ws")

    assert ok is True
    rsync = next(c for c in commands if c[0] == "rsync")
    assert rsync[-1] == "pop-os:/remote/ws/"
...
async def test_push_workspaces_noops_without_root(
    ...
    assert await backup._push_workspaces("pop-os", "/remote/ws") is False
    run.assert_not_awaited()
...
async def test_host_reachable_reflects_run_exit_code(
    ...
    assert await backup._host_reachable("pop-os") is True
    monkeypatch.setattr(backup, "_run", AsyncMock(return_value=255))
    assert await backup._host_reachable("pop-os") is False
