...
def test_settings_remote_section_configured_shows_toggle(
    ...
    from app.routes import settings as settings_routes
    monkeypatch.setattr(
        settings_routes, "remote_ollama_host", lambda: "http://pop-os:11434"
    )
    monkeypatch.setattr(
    ...
    assert "http://pop-os:11434" in response.text
...
def test_agent_overrides_respects_remote_toggle(tmp_path: Path) -> None:
    ...
    erroring or trying to reach pop-os."""
    ...
    try:
        with open_connection(db_path) as conn:
            ...
            assert ov["ollama_host"] == "http://pop-os:11434"
...
def test_settings_add_server_health_fail_returns_502(
    ...
    with make_client(_ollama_unreachable) as client:
        response = client.post(
            "/settings/servers",
            data={"name": "arxiv_rag", "url": "http://pop-os:8002/arxiv_rag", "description": "test"},
        )
...
def test_settings_add_server_health_unknown_name_returns_502(
    ...
    with make_client(_ollama_unreachable) as client:
        response = client.post(
            "/settings/servers",
            data={"name": "bogus_rag", "url": "http://pop-os:8002/bogus_rag", "description": "test"},
        )
    ...
    with make_client(_ollama_unreachable) as client:
        ...
        with open_connection(db_path) as conn:
            queries.append_message(
                ...
                json.dumps({"name": "current_time", "arguments": {}}),
            )
            queries.append_message(
