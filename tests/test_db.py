...
def test_migration_backfills_think_mode_column(tmp_path: Path) -> None:
    """A conversations table that pre-dates phase 25 gets the think_mode
    column backfilled (default 'default') on init, idempotently."""
    db = tmp_path / "chats.db"
    with sqlite3.connect(db) as conn:
        conn.executescript(
            """
...
def test_messages_thinking_column_present_on_fresh_db(
    initialized_db: Path,
) -> None:
    """Phase 28: a fresh DB's messages table includes the `thinking` column."""
    with _open(initialized_db) as conn:
        columns = {
            row[1] for row in conn.execute("PRAGMA table_info(messages);")
        }
...
def test_migration_backfills_thinking_column(tmp_path: Path) -> None:
    """Phase 28: a messages table that pre-dates this phase gets the
    nullable `thinking` column backfilled on init (NULL = no reasoning).
    """
    db = tmp_path / "chats.db"
    with sqlite3.connect(db) as conn:
        conn.executescript(
    ...
    with sqlite3.connect(db) as conn:
        ...
        assert "thinking" in columns
        value = conn.execute(
            "SELECT thinking FROM messages WHERE content = 'legacy row';"
        ).fetchone()[0]
        assert value is None
